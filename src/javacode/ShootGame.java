...
public class ShootGame extends JPanel {
    ...
    public void paintScore(Graphics g) {
        ...
        y=y+20;
        g.drawString("DF:" + hero.isDoubleFire(),x,y); //画双倍火力弹数
    }
    ...
    public void paintState(Graphics g) {
        switch (state) {
            case START: // 启动状态
                g.drawImage(start, 100, 270, null);
                break;
            case PAUSE: // 暂停状态
                g.drawImage(pause, 100, 270, null);
                break;
            case GAME_OVER: // 游戏终止状态
                g.drawImage(gameover, 100, 270, null);
                break;
        }
    }
    ...
    public void bangAction() {
        for (int i = 0; i < bullets.length; i++) { // 遍历所有子弹
            Bullet b = bullets[i];
            int bitoN;
            bitoN = bang(b); // 子弹和飞行物之间的碰撞检查
            if(bitoN > 0){

                Bullet temp = bullets[i]; // 击中的子弹与最后一个子弹交换
                bullets[i] = bullets[bullets.length - 1];
                bullets[bullets.length - 1] = temp;

                bullets = Arrays.copyOf(bullets, bullets.length - 1); // 删除最后一个子弹(即击中的)
            }
        }
    }
